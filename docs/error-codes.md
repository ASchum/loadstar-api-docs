...
> ⚠️ **Этот файл — рекомендация, а не обязательный контракт.**  
> Источник истины — [`openapi.yaml`](openapi.yaml). Где есть расхождение между файлами — доверяйте `openapi.yaml`.  
> Примеры кодов TypeScript и React-компонентов — предложения, адаптируйте под свой фреймворк.
...
// Токен поддельный или уже в blacklist
{
  "error": "Token Invalid",
...
{
  "error": "Token Expired",
  "message": "Токен истёк, выполните refresh",
  "details": null
}
```
...
### 403 Forbidden
**Когда:** Токен валидный, но недостаточно прав или аккаунт деактивирован

**Варианты:**
```json
// Недостаточно прав (роль не подходит)
{
  "error": "Forbidden",
...
**Фронтенд действие:**
- `Forbidden` → показать сообщение, скрыть недоступные кнопки/разделы
- `Account Disabled` → удалить токен, редирект на /login с сообщением "Ваша учётная запись деактивирована"
...
### Лид не дозаполнен перед одобрением запроса (реальный пример)
```json
...
**Фронтенд действие:** открыть форму дозаполнения карточки лида, подсветить перечисленные в `details.missing_client_fields` поля, после сохранения — повторить `approve()`.
...
**Документация:**
- Источник истины — `openapi.yaml`. Этот файл (error-codes.md) содержит рекомендации; при расхождении доверяйте `openapi.yaml`
