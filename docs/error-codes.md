...
5. Frontend показывает диалог: "Данные изменены Иван Иванов. Загрузить актуальные?"

**Фронтенд действие:**
- **Показать диалог** с предупреждением и именем пользователя, который изменил
- **Кнопка "Загрузить актуальные данные":**
  - Делает `GET /api/clients/123`
  - Получает свежую версию с новым `updated_at`
  - **Перезаполняет форму** актуальными данными
  - **Форма остается открытой** — пользователь видит, что изменилось
  - Может продолжить редактирование поверх свежих данных
- **Кнопка "Отменить":**
  - **Закрывает форму** (модальное окно)
  - Возврат к списку клиентов
  - Все несохраненные изменения текущего пользователя теряются
- **НЕ пытаться** автоматически перезаписать данные без ведома пользователя
...
```typescript
// Конфликт версий, удаление заблокировано
<Modal>
  <h2>{error.error}</h2>
  <p>{error.message}</p>
