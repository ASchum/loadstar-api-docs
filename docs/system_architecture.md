...
- **Временная зона:** UTC (+00:00)
  - Все timestamp хранятся в UTC
  - Конфигурация: `default-time-zone = '+00:00'`
  - Исключает проблемы с летним/зимним временем
  - Упрощает работу с международными перевозками
...
Sales Менеджер
  ...
  │  ├─ Исполнение заказа может стартовать до получения полного пакета договоров/документов
  │  └─ Договоры и документы могут добавляться по мере сопровождения
  │
