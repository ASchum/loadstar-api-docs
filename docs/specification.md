...
**Описание:** Суперпользователь. Управление системой, пользователями и справочниками, а также полный доступ ко всем рабочим данным наравне с Supervisor.
...
- Просмотр и редактирование всех клиентов, запросов и заказов
- Одобрение и завершение любых заказов

**Доступ к функциям:** Полный (управление системой + все рабочие данные)
...
- **Редактирование:** Sales (только свои лиды со статусом Lead), Ops/Supervisor (все клиенты)
- **Просмотр карточки клиента:** Ops, Supervisor, Admin, Logist (Sales не имеет доступа к общему списку; работа с лидами ведётся через интерфейс Запроса)
- **Изменение статуса:**
...
- Sales не имеет доступа к списку лидов/клиентов; при создании запроса Sales всегда вводит данные нового лида
- Активные клиенты (Active) доступны для выбора всем (Ops, Supervisor)
- Sales редактирует лида только пока `status = Lead`; после конвертации в Active карточкой управляет Ops
...
- Назначенный логист (выбор из списка Logist, необязательно)
- Клиент/Лид (связь)
- Дата создания
...
**Операции:**
- Создание запроса (Sales, Ops, Supervisor) — можно указать logist_id при создании
- Редактирование активного запроса (Sales/Ops — свои, Supervisor — все) — включая смену logist_id
...
- **`is_route_negotiated`** (логический флаг, default: false)
  - **Смысл:** Согласован ли маршрут доставки с логистом
  - **Значение true:** Маршрут обсужден в чате согласования, логист подтвердил возможность
  - **Значение false:** Маршрут ещё не согласован или требует переговоров
  - **Заполняет:** Sales/Ops (вручную по своим запросам по итогам переписки в чате) или Admin/Supervisor (по любому запросу). Logist флаг установить не может.
- **`signet_at`** (timestamp, default: NULL)
  - **Смысл:** Дата/время, когда заявка согласована и готова к запуску в исполнение
  - **Значение NULL:** Заявка не подписана (не согласована для исполнения)
  - **Заполненное значение:** Заявка подписана (готова к запуску), содержит дату согласования
  - **Заполняет:** Ops (по своим запросам) или Supervisor (любые запросы) через отдельное действие
  - **Предусловия для установки:** ≥1 маршрутный этап (`route_stage`), ≥1 тип ТС (`request_unit`), основные поля заполнены
...
- **Фактические доходы/расходы (Financial Records):**
  - Приход или Расход
  - Контрагент (только для Расхода, необязательное поле)
  - Описание услуги
  - Количество услуг
  - Цена за единицу
  - Итог (вычисляется)
...
- Sales не выбирает лида из списка; при создании запроса Sales всегда вводит данные нового лида
- Активные клиенты (Active) доступны всем (Ops, Supervisor)

### 4.4 Согласование маршрута vs Согласование заявки (Различие флагов)

| Параметр | `is_route_negotiated` | `signet_at` |
|----------|----------------------|-------------|
| **Место хранения** | Таблица `request`, логический флаг (boolean) | Таблица `request`, timestamp (может быть NULL) |
| **Смысл** | Согласован ли маршрут с логистом | Готова ли заявка к запуску в исполнение |
| **Значении false/NULL** | `false` = маршрут не согласован, требует переговоров | `NULL` = заявка не подписана, не готова к исполнению |
| **Значение true/FILLED** | `true` = маршрут обсужден, логист подтвердил возможность | Timestamp = заявка подписана, готова к запуску |
| **Кто может устанавливать** | Sales/Ops вручную по своим запросам; Admin/Supervisor по любому; Logist — нет | Только Ops (по своим запросам) или Supervisor (по любым) через отдельное действие |
| **Предусловия установки** | Sales/Ops переписались с логистом в чате и получили подтверждение | ≥1 маршрутный этап, ≥1 тип ТС, основные поля заполнены |
| **Откат (обнуление)** | Только Admin/Supervisor, через `POST /orders/{id}/reopen` — сбрасывает `is_route_negotiated` в `false`. Статус остаётся `Approved`, заказ **не** возвращается инициатору и не переходит в `Negotiating` | Только Ops (по своим) или Supervisor может очистить в NULL |
| **Влияние на `status`** | Не влияет; `status` меняется отдельно и независимо | Не влияет; `status` меняется отдельно и независимо |
| **Пример использования** | Фильтрация запросов, готовых к переходу в Approved | Фильтрация заказов, готовых к запуску операций (план. отправление, юр. оформление) |

### 4.5 Обязательные бизнес-правила
...
- Запрос должен содержать хотя бы 1 договор (`request_contract`) — добавляется Sales через `POST /requests/{id}/contracts` до одобрения
- Для Лида: перед одобрением Sales обязан заполнить в карточке лида: ИНН, банковские реквизиты (bank_name, bank_account, correspondent_account, bik) и ведущего менеджера (leading_manager_id); только после этого статус автоматически меняется Lead → Active
- Документы не являются блокером перехода в Approved — дозагружаются в процессе сопровождения заказа

**Сброс согласования маршрута (`POST /orders/{id}/reopen`):**
- Только Admin/Supervisor
- Статус остаётся `Approved` — запись не покидает секцию заказов
- Сбрасывает `is_route_negotiated = false`; Supervisor корректирует маршрут самостоятельно
- `signet_at` не сбрасывается
...
- **Если от Sales:**
  - Ввод данных нового Лида (минимум: имя; остальные поля заполняются позже перед переводом в заказ)
  - Ввод Данных Запроса
  - Ввод Пожеланий Лида
...
**UX при конфликте:**
Frontend показывает модальный диалог с сообщением:
- **Сообщение**: "Данные были изменены другим пользователем с момента открытия формы"
- **Версии**: Показать вашу версию и текущую версию (timestamp) для информации
- **Кнопка**: "Обновить страницу" - страница перезагружается
- Пользователь видит актуальные данные и может внести свои изменения заново
...
- **СУБД:** MySQL 8.0+
- **Backend:** Laravel 12 (PHP 8.2+), JWT-аутентификация, задеплоен на `api.loadstar.xd0.ru`


### 7.5 Работа с датами и временем

**Все временные метки (timestamp) хранятся в UTC (Coordinated Universal Time, +00:00).**

**Обоснование:**
- Система работает с международными перевозками по всему миру
- Грузы проходят через разные временные зоны
- UTC исключает проблемы с переходом на летнее/зимнее время
- Упрощает синхронизацию данных между разными локациями

**Технические поля с timestamp:**
- `created_at` - дата и время создания записи (UTC)
- `updated_at` - дата и время последнего обновления (UTC)
- `completed_at` - дата и время завершения заказа (UTC)
- `converted_at` - дата и время конвертации лида в клиента (UTC)
...
- Для полей DATE: `"2026-02-17"` (без времени)

**Конфигурация MySQL:**
```sql
-- В конфигурационном файле my.cnf / my.ini
[mysqld]
