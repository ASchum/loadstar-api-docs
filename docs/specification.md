...
- Редактирование данных сотрудника (ФИО, роль, контакты, активность)
...
### 3.1 Сущность: Сотрудник (Employee)
...
### 3.2 Сущность: Контрагент (Counterparty)
...
**Описание:** Существующие клиенты компании.
...
- Категория клиента (A, B, C, D)
- Направления перевозок (текст)
- С кем работают (текст)
- Что ценят (текст)
- Текущий контакт (один или несколько):
  - ФИО
  - Должность
  - Email (валидация)
  - Телефон (валидация)
- ЛПР — Лицо, принимающее решение (один или несколько):
  - ФИО
  - Должность
  - Email (валидация)
  - Телефон (валидация)
...
**Примечание:** Неактивные клиенты не отображаются в списках для поиска.

---

### 3.4 Сущность: Лид (Lead)
**Описание:** Потенциальный клиент, с которым работает Sales.
...
- Адрес
- Комментарий
- Статус (Потенциальный, В переговорах, Переведен в клиента, Отказано)

**Примечание:** Лид может стать Клиентом. При переводе Лида в Клиента заполняются недостающие реквизиты (банковские данные и т.д.).

---

### 3.5 Сущность: Запрос (Request)
**Описание:** Запрос на доставку груза. Может быть инициирован Sales (от Лида) или Ops (от Клиента).
...
## 4. Требования к функциям и интерфейсам

### 4.1 Интерфейсы для Work Items (Карточки)

#### 4.1.1 Управление Сотрудниками (Admin)
- **Список сотрудников:** таблица со всеми сотрудниками
- **Добавление сотрудника:** форма с полями: ФИО, роль, телефон, email, логин, пароль
- **Редактирование сотрудника:** редактирование всех полей
- **Деактивация:** смена статуса на "Неактивный"

#### 4.1.2 Управление Контрагентами (Ops, Supervisor)
...
#### 4.1.3 Управление Клиентами (Ops, Supervisor)
...
#### 4.1.4 Управление Словарями (Admin)
...
### 4.2 Интерфейсы для Запросов (Request)

#### 4.2.1 Список Запросов (Sales, Ops, Supervisor, Logist)
...
#### 4.2.3 Просмотр и Редактирование Запроса
- **Активный запрос:**
...
#### 4.2.4 Согласование Маршрута (Sales/Ops + Logist)
- **Чат интерфейс:**
...
#### 4.2.5 Построение Окончательного Маршрута (Sales/Ops)
...
### 4.3 Интерфейсы для Заказов (Order)

#### 4.3.1 Список Заказов (Ops, Supervisor)
...
#### 4.3.2 Просмотр и Редактирование Заказа (Ops)
- **Активный заказ:**
...
#### 4.3.3 Контроль Маршрута (Supervisor)
- **Запрос на изменение маршрута:**
...
#### 4.3.4 Управление Договорами на Заказе
- **Таблица договоров:**
  - Номер договора
  - Дата заключения
  - Условия оплаты
...
### 4.4 Отчеты и Статистика (Supervisor)
...
### 6.4 Технологический стек (планируемый)
- **СУБД:** MySQL
