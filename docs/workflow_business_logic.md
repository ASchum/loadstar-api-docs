...
- **Запрос переходит в статус Approved** после согласования маршрута (маршрут не имеет собственного статуса)
- **Удалено поле** `route_negotiation_status` (boolean) — согласование определяется статусом запроса
